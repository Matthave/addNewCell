...
class Main extends React.Component {
  ...
  addCell = (event) => {
    ...
    if (popupInputValue === "" || popupInputValue.length < 2) {
      alert(
        "Invalid cell name! The minimum number of characters is 2 characters"
      );
      return;
    } else if (popupInputValue.length > 100) {
      alert(
        `Invalid cell name! the maximum number of characters is 100 characters. You entered ${popupInputValue.length} characters`
      );
      return;
    }
  ...
  addEthniCell = (event) => {
    ...
    if (ethniInputValue === "" || ethniInputValue.length < 2) {
      ...
      alert(
        `Invalid cell name! the maximum number of characters is 50 characters. You entered ${ethniInputValue.length} characters`
      );
      return;
    }
  ...
  removeCell = (id) => {
    ...
    const newCells = currentCells.filter((cell) => cell.id !== id);
    this.setState({
      cells: newCells,
    });
  };
    ...
    return (
      <>
        <main className="main">
          ...
          {removeDefaultOne ? null : (
            <Age removeDefaultCells={this.removeDefaultCells} />
          )}
          ...
          {removeDefaultThree ? null : (
            <DefaultThree removeDefaultCells={this.removeDefaultCells} />
          )}

          {newCell}
          <button
            className="addBtn"
            onClick={() => this.clickHandlePopup("show")}
          >
